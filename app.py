...
# Se o botão for clicado, atualiza a página
if st.button("Avançar para análise completa"):
    st.session_state.page = "análise_completa"

    # Suponha que você tenha essas variáveis já preenchidas com os dados do usuário
    respostas = (respostas_binarias, respostas_esg, respostas_financeiros)
    nome_empresa = "Empresa"

    # Avaliação (supondo que esta função já exista no seu código)
    df_resultados, total, score_esg, score_fin = avaliar_empresa(nome_empresa, respostas)

    # Radar ESG x Financeiro
    def plotar_radar(df_resultados, nome_empresa):
        categorias = df_resultados['Indicador']
        valores = df_resultados['Score']
        categorias = list(categorias)
    ...
    # Gráfico de melhoria do score financeiro com aumento do ESG
    def plotar_impacto_melhoria_esg(score_esg, score_fin, nome_empresa):
        ...
        ax.set_title(f'Estimativa de Melhoria Financeira com ESG - {nome_empresa}', fontsize=14, weight='bold')
    ...
    # Gráfico de impacto das práticas ESG em EBITDA e Receita
    def plotar_impacto_praticas_esg():
        praticas = [
            "Energia Renovável", "Diversidade de Gênero", "Ética na Cadeia",
            "Satisfação dos Funcionários", "Redução de Carbono"
        ]
        impacto_ebitda = [3, 3, 4, 6, 2]
        impacto_receita = [0, 2, 0, 5, 1]
        x = range(len(praticas))
    ...
    # Projeção do EBITDA com e sem ESG
    def plotar_projecao_ebitda():
        anos = [2025, 2026, 2027, 2028, 2029]
        ...
        ax.plot(anos, ebitda_esg, marker='o', label='Com Melhoria ESG')
        ax.set_xlabel('Ano')
    ...
    # Comparativo histórico ESG vs Não ESG
    def plotar_comparativo_esg_nao_esg():
        ...
        def acumulado(retorno, dy):
            v = [valor_inicial]
            for r in retorno:
                v.append(v[-1] * (1 + r + dy))
            return v[1:]

        v_esg = acumulado(retorno_esg, dy_esg)
        v_nao_esg = acumulado(retorno_nao_esg, dy_nao_esg)
        v_ise = acumulado(retorno_ise, dy_ise)

        fig, ax = plt.subplots(figsize=(12, 7))
        ax.plot(anos, v_esg, marker='o', label='Empresas ESG', color='green')
        ax.plot(anos, v_nao_esg, marker='s', label='Sem ESG', color='red')
        ax.plot(anos, v_ise, marker='^', label='ISE B3', color='blue')
        ax.set_title('Valor Acumulado das Ações com Dividendos (2018-2022)')
        ax.set_xlabel('Ano')
    ...
    plotar_impacto_praticas_esg()
    plotar_projecao_ebitda()
    plotar_comparativo_esg_nao_esg()
